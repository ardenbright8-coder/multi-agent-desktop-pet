...
- 🚨🚨 **「点得动」不等于「看得见」。** `click()` 点之前会 `scrollIntoView()` 自动滚过去，
  于是控件哪怕被挤出视野，测试照样绿 —— **测试把用户不会做的事替它做了**。
  2026-08-12 实机踩到：面板字号放大后选项滚出视野，用户点确认毫无反应（错误提示也在滚动区里看不见），
  而 `controls` 全绿。凡是「用户得先看到才能操作」的东西，断言要写成
  **「面板一打开它就在视野内」**，不能只验「点得动」。
