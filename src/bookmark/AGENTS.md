...
| `launch.ts` | 启动 Agent 开窗（设定15第二版）：`agentShortcutFor` 组名→桌面快捷方式（Claude→`Claude Code 一`、ChatGPT→`Codex`、Pi Agent→`Pi 编程智能体`、Hermes→`Hermes`），纯函数；打开在 panel-window `openAgentShortcut`，**只开窗，不贴字不回车**；自测不常驻＝只报开哪个不真开 |
...
5. `node scripts/verify-bookmark-copy.mjs` 过（行尾复制、六个点启动 Agent（含提示开了哪个快捷方式）、右键精简、组头图标、官方标志、捆一捆、真敲命令领活/done/放回去；会动系统剪贴板，脚本自己存了还原）
