...
- 🚨 **单击和拖动分开（2026-09-24 用户拍板，设定 11）**：鼠标按下挪过 5 像素就是拖（不用等，`BM_DRAG_SLOP`）；没挪就松手是单击，**松手马上响应**（条目文字单击改字、项目页单击进夹/打开 md），按多久都算单击；拖完那一下点击被 `row.dataset.swallow` 吞掉。🚫 别改回「按住 0.3 秒才能拖」（人按下就挪，会被判成单击直接进改字）；🚫 别改成双击、别给单击加延时（当天试过双击，用户：「单击点了半天没反应」）。不走系统拖放，不靠左边小点（小点只管「派给 / 删除」）；别给条目加 `draggable`。待定、分组条目、组标题、项目页同一个 `bmBindDrag`。
...
已知基线（2026-09-24，改之前就红，不算新红）：`verify-bookmark-tabs.mjs` 找不到旧输入框 `#bm-inbox-input`；`verify-bookmark-handoff.mjs` 4 项（专区位置、详情展开/收起、空态提示）——都是 `3a981cc` 长按拖动改版拿掉了旧控件，脚本没跟上。
