...
- 🚨 **单击和拖动分开（2026-09-24 用户拍板，设定 11）**：鼠标按下挪过 5 像素就是拖（不用等，`BM_DRAG_SLOP`），没挪就松开才算单击；**单击不弹改字、不进夹**，双击条目文字才改字、双击项目页文件夹/md 才进/才打开。别改回「按住 0.3 秒才能拖」——人都是按下就挪，会被判成单击直接进改字。不走系统拖放，也不靠左边小点（小点只管「派给 / 删除」）；别给条目加 `draggable`，会画禁止符号。待定、分组条目、组标题、项目页同一个 `bmBindDrag`。
...
- 🚨 **交接单条目（kind=handoff）渲染必须保持显眼标记（📋+淡黄底）**——用户拍板「一眼认出」；**交接单专区固定在 Agent分组页最底部、＋加Agent 按钮上方，别处不加**（用户画的黄框位置，2026-09-10 拍板）；未派发交接单默认落专区，拖到哪个组头=派给谁，组内交接单可「收回交接单专区」；「接完活自己 remove 删掉」的规矩草案写在仓库根 AGENT_INTEGRATION.md 的 CLI 节，别删了它。
...
已知基线（2026-09-24，改之前就红，不算新红）：`verify-bookmark-tabs.mjs` 找不到旧输入框 `#bm-inbox-input`；`verify-bookmark-handoff.mjs` 2 项（专区位置、空态提示；详情展开/收起 2026-09-24 单击不改字后已转绿）——都是 `3a981cc` 长按拖动改版拿掉了旧控件，脚本没跟上。
...
5. `node scripts/verify-bookmark-drag.mjs` 过（拖动时看板重画不留残影；按下直接挪就拖、单击不改字、双击改字、项目页双击才进）
