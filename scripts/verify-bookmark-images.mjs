...
try {
  ...
  await page.locator('.group[data-group="Claude"] .g-add').click();
  await page.locator(".insert-row textarea").fill("按钮改这里，下面保留");
  await page.evaluate(() => {const t=document.querySelector(".insert-row textarea"); t.focus();t.setSelectionRange(6,6);});
  await paste(page);
  ...
  await page.evaluate(() => {const input=document.querySelector(".insert-row textarea"); input.focus();input.setSelectionRange(input.value.length,input.value.length);});
